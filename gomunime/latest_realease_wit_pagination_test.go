...
type LinkableLatestPagination struct {
	Name string `json:"name"`
	URL  string `json:"url"`
}

type AnimeLatestPagination struct {
	...
	Studio    []LinkableLatestPagination `json:"studio,omitempty"` // [IMPROVED] Bisa menampung banyak studio
	Genres    []LinkableLatestPagination `json:"genres,omitempty"` // [IMPROVED] Menyimpan nama dan URL genre
}
...
func TestLatestPagination(t *testing.T) {
	...
	var allAnime []AnimeLatestPagination
	...
	c.OnHTML("div.ingfo", func(e *colly.HTMLElement) {
		...
		animeInfo := e.Request.Ctx.GetAny("anime").(AnimeLatestPagination)
		...
		e.ForEach(".linginfo span", func(_ int, s *colly.HTMLElement) {
			...
			if strings.HasPrefix(text, "Status:") {
				...
				s.ForEach("a", func(_ int, a *colly.HTMLElement) {
					animeInfo.Genres = append(animeInfo.Genres, LinkableLatestPagination{
						Name: a.Text,
				...
				s.ForEach("a", func(_ int, a *colly.HTMLElement) {
					animeInfo.Studio = append(animeInfo.Studio, LinkableLatestPagination{
						Name: a.Text,
						URL:  e.Request.AbsoluteURL(a.Attr("href")),
					})
				})
	...
	c.OnHTML("div.listupd article.bs", func(e *colly.HTMLElement) {
		...
		anime := AnimeLatestPagination{
			...
			Genres: make([]LinkableLatestPagination, 0),
			Studio: make([]LinkableLatestPagination, 0),
		}
	...
	// Callback #3: Menangani pagination
	c.OnHTML("div.hpage a.r", func(e *colly.HTMLElement) {
		if currentPage >= maxPage {
			log.Printf("Batas maksimal halaman tercapai: %d", maxPage)
			return
		}
		currentPage++
		nextPageURL := e.Attr("href")
		log.Printf("Menuju halaman berikutnya (%d): %s", currentPage, nextPageURL)
		c.Visit(nextPageURL)
	})
	...
	log.Printf("Memulai proses scraping, maksimal %d halaman...", maxPage)
	c.Visit(baseURL)
	c.Wait()
